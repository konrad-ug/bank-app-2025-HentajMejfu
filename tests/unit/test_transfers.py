...
class TestPersonalTransfers:
    ...
    @pytest.mark.parametrize(
        "balance, amount, expected_result, expected_balance, expected_history",
        [(100, 50, True, 50, [-50]), (30, 50, False, 30, [])],
        ids=["enough balance", "not enough balance"],
    )
    def test_normal_transfer(
        self,
        pacc: PersonalAccount,
        balance: float,
        amount: float,
    ...
    @pytest.mark.parametrize(
        "balance, amount, expected_result, expected_balance, expected_history",
        [(50, 50, True, -1, [-50, -1]), (30, 50, False, 30, [])],
        ids=["enough balance", "not enough balance"],
    )
    ...
    def test_receive_transfer(self):
        acc: PersonalAccount = PersonalAccount(
            "Pawel", "Sobolewski", "12345678901", "PROM_4200"
        )
...
class TestBusinessTransfers:
    @pytest.fixture()
    def bacc(self, mocker: MockerFixture):
    ...
    @pytest.mark.parametrize(
        "balance, amount, expected_result, expected_balance, expected_history",
        [(100, 20, True, 80, [-20]), (10, 20, False, 10, [])],
        ids=["enough balance", "not enough balance"],
    )
    def test_normal_transfer(
        self,
        bacc: BusinessAccount,
        balance: float,
        amount: float,
        expected_result: bool,
        expected_balance: float,
    ...
    @pytest.mark.parametrize(
        "balance, amount, expected_result, expected_balance, expected_history",
        [(50, 50, True, -5, [-50, -5]), (30, 50, False, 30, [])],
        ids=[
    ...
    def test_receive_transfer(self, bacc: BusinessAccount):
        assert bacc.receiveTransfer(69)
        assert bacc.balance == 69
        assert bacc.history == [69]
